...
from flaskproj.users.forms import (RegistrationForm, LoginForm, UpdateForm,
                                   RequestResetForm, ResetPasswordForm)
from flaskproj.users.utils import save_pic, send_reset_email
...
def reset_request():
    ...
    if form.validate_on_submit():
        ...
        flash('An email has been sent owo', 'info')
        return redirect(url_for('users.login'))
...
def reset_token(token):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(f'Password was updated for {user.username}', 'success')
        return redirect(url_for('users.login'))
